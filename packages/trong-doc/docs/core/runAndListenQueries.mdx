---
sidebar_position: 1
slug: /core/run-and-listen-queries
---

# Run and listen queries

You can use `trong-orm` without react or any other libs integrations.
...
const db = await initDbClient({
  ...
  dbBackend: absurdWebBackend({
    wasmUrl: "https://trong-orm.netlify.app/wasm/sql-wasm.wasm",
  }),
  plugins: [
...
const subscription = listenQueries(db, [select().from(notesTable)]).subscribe(
  (res) => {
